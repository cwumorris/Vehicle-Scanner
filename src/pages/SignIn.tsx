...
const SignIn = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const auth = login(username.trim(), password);
      if (!auth) {
        toast({ title: "Login failed", description: "Invalid credentials", variant: "destructive" as any });
        return;
      }
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
          <form className="space-y-4" onSubmit={onSubmit}>
            ...
            <div className="text-xs text-muted-foreground mt-2">
              Admin: admin / admin123 · Guard: guard / guard123
            </div>
          </form>
        </div>
      </div>
