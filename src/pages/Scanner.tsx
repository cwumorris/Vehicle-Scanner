...
function Scanner() {
  ...
  const bgClass = result ? (result.approved ? "bg-green-600" : "bg-red-700") : "bg-black";

  return (
    <div className={`w-full h-full ${bgClass} text-white flex items-stretch`}>
      ...
      <div className="flex-1 flex flex-col justify-center px-4 md:px-8">
        <div className="w-full bg-white/10 backdrop-blur rounded-xl p-4 md:p-6 shadow-lg">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              if (input.trim()) {
          ...
          >
            <input
              ...
              className="flex-1 h-14 md:h-16 px-5 md:px-6 rounded-lg bg-black/30 text-white placeholder-white/60 border border-white/20 focus:outline-none focus:ring-4 focus:ring-white/20 text-lg md:text-xl"
              ref={inputRef}
            />
            <button
              type="submit"
        ...
        {showPopup && result && (
          <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/80 z-50" onClick={resetScanner}>
            <div className="bg-white rounded-xl p-6 max-w-md w-full text-gray-900 shadow-2xl" onClick={e => e.stopPropagation()}>
